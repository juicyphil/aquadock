...
export function TankDetailView({ tank, onUpdated, onDelete }: Props) {
  ...
  const triggerQuickAction = (actionTab: Tab, showForm?: boolean) => {
    setTab(actionTab)
    if (actionTab === 'inhabitants') setShowAddInhab(true)
    else if (actionTab === 'events') setShowAddEvent(true)
    else if (actionTab === 'params') setShowLogParam(true)
  }
  ...
  return (
    <div>
      ...
      <div className="tank-detail-header">
        ...
        <div className="tank-detail-info">
          <div className="tank-detail-name">{tank.emoji} {tank.name}</div>
          <div className="tank-detail-meta">
            {tank.liters}L &middot; {tr(tank.type)}{tank.subtype ? ` · ${tr(tank.subtype)}` : ''}
            &middot; {tr('tank.setup_date')}: {tank.setup_date}
          </div>
        </div>
        <div className="form-actions" style={{ flexShrink: 0 }}>
          <button className="btn btn-primary btn-sm" onClick={() => setEditing(true)}>{tr('tank.edit')}</button>
          <button className="btn btn-danger btn-sm" onClick={onDelete}>{tr('tank.delete')}</button>
        </div>
      </div>

      {/* Quick Actions */}
      <div className="quick-actions">
        <button className="btn btn-primary" onClick={() => triggerQuickAction('params')}>
          🔬 {tr('param.log')}
        </button>
        <button className="btn btn-primary" onClick={() => triggerQuickAction('events')}>
          📅 {tr('event.schedule')}
        </button>
        <button className="btn btn-primary" onClick={() => triggerQuickAction('inhabitants')}>
          🐟 {tr('inhabitant.add')}
        </button>
      </div>
      ...
      <div className="tab-content">
        ...
        {tab === 'inhabitants' && (
          <div className="detail-section">
            <div className="detail-section-header">
              <span className="detail-section-title">{tr('tank.inhabitants')}</span>
              <button className="btn btn-primary btn-sm" onClick={() => setShowAddInhab(!showAddInhab)}>
                + {tr('inhabitant.add')}
              </button>
            </div>
        ...
        {tab === 'events' && (
          <div className="detail-section">
            <div className="detail-section-header">
              <span className="detail-section-title">{tr('nav.planner')}</span>
        ...
        {tab === 'issues' && (
          <div className="detail-section">
            <div className="detail-section-header">
              <span className="detail-section-title">{tr('issue.title')}</span>
              <button className="btn btn-primary btn-sm" onClick={() => setShowAddIssue(!showAddIssue)}>
                + {tr('issue.add')}
              </button>
            </div>
            {issues.length === 0 ? (
        ...
        {tab === 'params' && (
          <div className="detail-section">
            <div className="detail-section-header">
              <span className="detail-section-title">{tr('param.history')}</span>
              <button className="btn btn-primary btn-sm" onClick={() => setShowLogParam(!showLogParam)}>
                + {tr('param.log')}
              </button>
            </div>
            {params.length === 0 ? (
